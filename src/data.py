...
def download_tif(
    ...
    *,
    ...
    ""
    if not remote_path.lower().endswith((".tif", ".tiff")):
        raise DownloadError(f"Remote path does not look like a TIFF: {remote_path}")

    # Build full URL safely
    url = urljoin(base_url.rstrip("/") + "/", remote_path.lstrip("/"))

    # Ensure destination directory
    os.makedirs(os.path.dirname(os.path.abspath(dest_path)), exist_ok=True)

    # Check if server supports HEAD (for size, content-type, resume)
    sess = requests.Session()
    req_headers = {"User-Agent": "vesuvius-rmoe/1.0"}
    if headers:
    ...
    content_type = None
    try:
        hr = sess.head(url, timeout=timeout, headers=req_headers, auth=auth, cookies=cookies, verify=verify_tls, allow_redirects=True)
        if hr.ok:
    ...
    if content_type and "tif" not in content_type.lower():
        # Some servers may serve generic octet-stream; warn but don't fail hard
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Download a TIFF file with resume & checksum.")
    ap.add_argument("--base", required=True, help="Base URL, e.g. https://data.server/")
    ap.add_argument("--remote", required=True, help="Remote path under base, e.g. volumes/scroll2/000123.tif")
    ap.add_argument("--out", required=True, help="Destination file path")
    ap.add_argument("--sha256", default=None, help="Expected SHA256 hex digest")
