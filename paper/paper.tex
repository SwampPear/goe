...
The input stem acts as a modality adapter for transforming input data into standardized latent and tokenized 
representations. Given an input batch, $x \in \mathbb{R}^{[B, C, \ldots]}$, with batch size $B$, channel count $C$, and 
remaining modality-dependent dimensions, $x$ should be mapped into the latent representation
\begin{equation}
  \begin{aligned}
    f_0 = \phi(x; \theta_s)
  \end{aligned}
