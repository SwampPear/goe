...
=======
The input stem acts as a modality adapter responsible for transforming data into a standardized latent and tokenized
representation. Regardless of data modality, the stem should perform three fundamental operations: \textit{projection}, 
\textit{normalization}, and \textit{tokenization}. Given an input batch
\[
\begin{aligned}
  x \in \mathbb{R}^{[B, N, \ldots]}
\end{aligned}
\]
\noindent with batch size $B$, channel count $C$, and remaining modality-dependent dimensions, the stem maps $x$ into a
...
\end{aligned}
\]
\noindent where $\phi(\cdot)$ is a parameterized feature extractor. For example the feature extractor could be a
convolutional stack for spatial data, a temporal encoder for sequences, or a feed-forward projection for tabular inputs.
Common building blocks should ensure local correlations are captured while preserving feature magnitude and typically 
are of the form 
$Linear/Conv/Embedding \rightarrow Norm \rightarrow Activation$.

The resulting latent $f_0$ is then tokenized by partitioning or sampling the representation into discrete tokens:
\[
\begin{aligned}
  t_i = W_t \psi(f_0, \Omega_i) + b_t, \hspace{16pt} T = \{t_1, \ldots, t_N\}
\end{aligned}
\]
where $\psi(\cdot, \Omega_i)$ extracts the subset, patch, or time segment corresponding to the $i$-th token and $W_t$
projects it into a shared embedding space of dimension $d$. This process yields a modality-independent token set
$T \in \mathbb{R}^{B, N, d}$. By decoupling the feature extraction and tokenization mechanisms from any specific input 
structure, the input stem acts as a general adapter capable of ingesting multimodal data into a coherent token space.

In addition to token generation, the input stem produces a compact set of auxiliary routing features 
$r \in \mathbb{R}^{B, N, d_r}$ derived from the same latent representation $f_0$. These features capture 
modality-specific cues that assist the graph router in discovering expert pathways. The pair $(T, r)$ thus forms the 
complete standardized output of the input stem, supplying both the content and contextual signals required for 
downstream expert selection.
>>>>>>> parent of 0d5cd95 (graph router architecture fleshed out)
\\
...
\end{equation}
=======
After tokenization, the encoder operates on the unified token set, $T = \{t_1,\ldots,t_N\}$, and learns contextual 
relationships among tokens regardless of their original modality. Its design emphasizes domain invariance, allowing for
encoding dependencies across modalities. Formally, the encoder applies a sequence of permutation-invariant 
transformations
\[
\begin{aligned}
  T' = \Phi(T; \theta_e)
\end{aligned}
\]
where $\Phi$ may consist of stacked self-attention layers followed by normalization and residual feed-forward networks.
\[
\begin{aligned}
  \text{LayerNorm} \rightarrow \text{MultiHeadAttention}(Q,K,V) \rightarrow \text{Residual} \rightarrow 
  \text{FeedForward} \rightarrow \text{Residual}
\end{aligned}
\]
>>>>>>> parent of 0d5cd95 (graph router architecture fleshed out)
...
=======
The modality-agnostic encoder functions as the system's universal relational core. It captures token dependencies in a 
invariantly to the original data modality, enabling shared reasoning across heterogeneous inputs. Downstream modules
operate on these enriched token embeddings, promoting flexibility and cross-domain transfer.
>>>>>>> parent of 0d5cd95 (graph router architecture fleshed out)
\\
...
=======
The graph router defines a dynamic computation graph
\[
\begin{aligned}
...
routing features $a_i$ a combined descriptor
\[
\begin{aligned}
  r_i = [t_i'][a_i]
\end{aligned}
\]
\noindent is used to compute an entry distribution over experts
>>>>>>> parent of 0d5cd95 (graph router architecture fleshed out)
\[
\begin{aligned}
